...
function AddImg({ productId }) {
 ...
 const handleDeleteProductImg = async (imageId) => {
   try {
     const response = await axios.delete(`http://localhost:8000/admin-seller/productsImg/${imageId}`, {
       headers: {
 ...
 const fetchProductImages = async () => {
   try {
     const res = await axios.get(`http://localhost:8000/admin-seller/getallproductimgs/${productId}`);
     if (res.data.success) {
   ...
   if (validateForm()) {
     try {
       ...
       const res = await axios.post(
         `http://localhost:8000/admin-seller/productsImage/${productId}`,
         formDataToSend,
         { headers }
       );
         ...
         {productImages.map((image, index) => (
           <div className="col-lg-4 col-md-6 mb-4" key={image._id}>
             <div className="bg-image hover-overlay ripple shadow-1-strong rounded position-relative">
               <img src={`http://localhost:8000/${image.image}`} alt="" className="w-100 h-auto img-cover" />
